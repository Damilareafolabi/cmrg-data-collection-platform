...
No APK was built in the current environment. The Flutter SDK and `adb` are not installed (`FLUTTER_NOT_FOUND`, `ADB_NOT_FOUND`), so build output, signing, installation, and real-device behavior remain **ENVIRONMENT TEST REQUIRED**. Do not treat the Flutter source tree as an APK.
