...
- [ ] **Environment Configuration (Transitioning from Simulated to Real Secrets in VS Code)**
  - [ ] Downloaded or copied `.env` from the platform's simulated secrets modal (`.env.cmrg-vscode`).
  - [ ] Strong `AUTH_SECRET` generated (64-character random hex replacing simulated value).
  - [ ] Real `DATABASE_URL` configured replacing simulated localhost URL.
  - [ ] Strong `CMRG_COLLECT_SYNC_SECRET` set for Android fleet handshake.
  - [ ] Strong `STORAGE_SECRET_KEY` configured for AES-256 media encryption.
  - [ ] Real `GEMINI_API_KEY` provided for AI assistance (if enabled).
  - [ ] Super-admin default password updated from bootstrap default.
